...
public class CameraFollow : MonoBehaviour, IFixedUpdate
{
    ...
    [BoxGroup("缓动相关"), LabelText("垂直轴缓动因子"), Range(0.1f, 1)]
    [ShowIf("isActiveVerticalAxis")]
    public float verticalAxisDamping = 0.5f;

    [BoxGroup("缓动相关"), LabelText("水平缓动因子"), Range(0.0f, 1)]
    public float horizontalDamping = 0.5f;

    [BoxGroup("缓动相关"), LabelText("垂直缓动因子"), Range(0.0f, 1)]
    ...
    private bool isStopFollowState;
    ...
    private void Awake() {//保证最先加入LateUpdate列表
    ...
    private void MomentFollow(){
        ...
        if(camerViewType == CamerViewType.SV){
            if(!CheckIsOutsideArea_Y(ForcedFollowCenter.y, forcedFollowY)){
                followPos.y = Mathf.Lerp(CameraPos.y, followPos.y, verticalDamping);
            }
            else{
            ...
            if(!CheckIsOutsideArea_Y(ForcedFollowCenter.z, forcedFollowY)){
                followPos.z = Mathf.Lerp(CameraPos.z, followPos.z, verticalDamping);
            }
            else{
